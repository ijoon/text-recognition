...
class SegImageGenerator:
    def __init__(self, img_dirpath, img_w, img_h,
        ...
        self.texts, self.img_paths = self.get_label_img_file_paths()

        self.n = len(self.img_paths)               # number of images
        self.indexes = list(range(self.n))
        self.cur_index = 0
        self.imgs = np.zeros((self.n, self.img_h, self.img_w))
        self.seq = iaa.Sequential([
            iaa.AdditiveGaussianNoise(scale=0.05*255),
            iaa.Affine(translate_px={"x": (-10, 10), "y":(-10, 10)}, mode=ia.ALL)
        ])

    ## samples의 이미지 목록들을 opencv로 읽어 저장하기, texts에는 label 저장

    def get_label_img_file_paths(self):

        img_paths = []
        texts = []
        for (path, dirs, files) in os.walk(self.img_dirpath):
            for file_name in files:
                ...
                texts.append(str(label).ljust(9))
    ...
    def next_batch(self):       ## batch size만큼 가져오기
        while True:
            ...
            images_aug = self.seq(images=X_data.astype(dtype=np.float32))
            # dict 형태로 복사
            inputs = {
                'the_input': images_aug,  # (bs, 128, 64, 1)
