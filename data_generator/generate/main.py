...
def combinate_chars(unit_chars: dict,
    ...
    imgs = []
    random_seq_len = np.random.randint(1, seq_length+1)
    label = ""
...
if __name__ == "__main__":
    ...
    for n in range(20):
        ...
        seq_img, label = combinate_chars(unit_files, (32, 64), 6, align)
        seq_img = augment_background(seq_img, bg_img, (32, 64), 6)
        print(label)
        seq_img = shear_img_randomly(seq_img)
        # cv2.imshow('img', seq_img)
        # cv2.waitKey(0)
        cv2.imwrite('results/{}_{}.jpg'.format(n, label), seq_img)
